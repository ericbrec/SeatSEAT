...
**SeatSEAT** is a Jupyter notebook and Excel template you can use to allocate season tickets fairly and optimally among a group of sports fans without a draft or auction.
...
4. Fill out the Date, Time, Opponent, and Type columns in the template with your home game schedule. The Type column indicates the type of ticket which maps to a ticket price. If all your game tickets have the same price, just put "A" in the Type column for every game.
...
8. Fill out the Type and Price columns under the names table with each type of ticket and how much one ticket of that type costs. If all your game tickets have the same price, just put an "A" for the Type and the price per ticket for the Price.
9. Send your season ticket group members the FullName table (copy/paste it into a message), asking them to verify the spelling of their names and their allocation of pairs and quads. Adjust the template accordingly.
...
2. Do not change anything in the sheet other than the Rank column (and optionally the QuadRank column), the tiebreaker rule, and any optional constraints you select.
...
10. Take delivery of your tickets, pay the balance due, and enjoy the season!